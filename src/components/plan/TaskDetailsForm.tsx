...
const taskSchema = z.object({
  ...
  estHours: z.coerce.number().min(0, 'Estimated hours must be positive'),
});
...
export function TaskDetailsForm({ task, onSave }: TaskDetailsFormProps) {
  ...
  const onSubmit: SubmitHandler<TaskFormData> = (data) => {
    if (task) {
      const updatedTask: PlanTask = {
        ...
        assignees: data.assignees.split(',').map(s => s.trim()),
        estHours: data.estHours,
      };
      onSave(updatedTask);
      reset({}, { keepValues: true }); // Reset dirty state after save
    }
  };
  if (!task) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground">Select a task from the grid above to see its details.</p>
        </CardContent>
      </Card>
    );
  }
  return (
    <Card>
      <CardHeader>
        <CardTitle>Task Details: <span className="font-normal">{task.name}</span></CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <Controller
                name="estHours"
                control={control}
                render={({ field }) => <Input id="estHours" type="number" {...field} className="mt-1" />}
              />
              {errors.estHours && <p className="text-sm text-red-500 mt-1">{errors.estHours.message}</p>}
            </div>
          </div>
          <div>
            ...
            />
            {errors.assignees && <p className="text-sm text-red-500 mt-1">{errors.assignees.message}</p>}
          </div>
